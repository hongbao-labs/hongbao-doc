...
一个发卡方从“想发资产”到“用户成功领取资产”，大致会经历下面几个步骤。
...
普通卡走 1→3→4→5→6→7 即可；任务卡会多出第 2 步活动设计与第 7 步任务解锁。若你想绕过 Web Dapp、直接走合约层或自建客户端，开发者团队可参考[开源仓库](https://github.com/hongbao-labs/contracts)。
...
> 链、资产合约、锁定金额、过期时间等参数不在这份清单里——它们在第 4 步「Lock」时于 Web Dapp 设置。每张卡的领取二维码由 `card_address` 推导（`https://hongbao.digital/_c?ea=<地址前 6 位>`）。
...
把实体卡寄 / 送给用户。怎么送是你的事——夹张贺卡、活动现场、快递邮寄都行。
