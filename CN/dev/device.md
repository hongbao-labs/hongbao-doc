...
- **极端去信任配置的发卡方**：用开源 CLI 或自实现 BLE 客户端做自助取卡 / 测试 / 审计，不依赖 Hongbao 任何运营服务
...
**spec 公开**——这是 Hongbao 去信任承诺的一部分。基于本文档，任何拿到卡的人都能自己实现一套客户端把资产取出来。这是合约去信任之外，硬件层去信任的硬保证。

**CLI 开源**——我们提供开源的命令行工具，不藏着掖着：懂技术的人（审计、测试、自建集成、自助取卡）可以直接用；普通持卡人的日常体验仍然走官方 App（也是与钱包 / 交易所 / Dapp 生态合作的接入点），不需要碰 CLI。

**遇到问题怎么办**——日常领取走官方 App / Web。如果你需要在不依赖官方前端的情况下自助取卡，用开源 CLI 按本 spec 操作；不熟悉命令行的，联系我们的技术支持团队 [TBD: 联系方式]。
...
如果你属于上面三类受众之一，需要自己实现一套 BLE 客户端（审计 / 测试 / 自助取卡 / 配合魔改合约的集成场景；或直接用开源 CLI），按下面的顺序走：
