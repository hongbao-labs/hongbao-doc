...
- 卡面定制方案：详见 [Customization](customization)
...
二维码默认指向 `https://hongbao.digital/...`，扫出来直接进 Hongbao 官方领取入口（Web 端自动适配，安装了 Hongbao App 会优先打开 App），不需要额外引导。持卡人入口统一由 Hongbao 维护，发卡方不另行托管或替换前端。

## 6. 持卡人领取

详见 [持卡人领取指南](../receiver/guide)。
...
- **跨链 deposit**：每条链各自独立。卡是 secp256k1 地址形态，跨 EVM 链同址，但卡只能签一次，同一张卡只能在一条链上锁、一条链上领。不要往同一张卡的地址在多条链同时锁资产，否则用户在哪条链上领，其他链的资产就再也取不出来（只能等过期赎回）