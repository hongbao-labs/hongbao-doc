...
本节面向想自建 App 的开发者，定义设备对外的接口契约：BLE 服务/特征值、二进制帧格式、命令表、状态机、二维码格式、签名流程。

> 固件代码闭源；这里描述的全部是**对外接口**。任何尊重本接口的客户端都能正确驱动设备。
...
10. **提交 withdraw**：自行提交、或让用户的钱包提交、或 POST 到任意 relayer

**不要做**的事：

- 不要在 App 里持有/创建用户钱包私钥——这不是我们的产品形态
- 不要假设设备能签多次，永远只能签一次
- 不要忽略 ECDSA `s` 值的低-S 规范化（OpenZeppelin 的 ECDSA / 标准 `ecrecover` 都会处理；自己实现的库要注意）
- NFT 场景务必校验 `to` 能接收 ERC721——签错卡就报废

## 参考实现

- 我们的 Web App（已上线）— [TBD: URL]
- 我们的 Android App（开发中）
- iOS App（Android 完成后移植）
