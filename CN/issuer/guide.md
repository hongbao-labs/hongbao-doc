...
> 普通卡（plain card）走 1→3→4→5→6→7（一次领全额）即可；任务卡（task card）多出第 2 步活动设计与第 7 步任务解锁。想绕过 Web Dapp、直接走合约层 / 自建客户端的开发者团队，参考 [dev/](../dev/)。本文面向项目方运营 / 商务侧，按 Web Dapp 流程描述。
...
- **设置基础份额 + 任务列表**：每个任务一个金额 + 一个完成条件（关注 / 转推 / 进群 / 链上活跃度认证等，最多 255 个）
- **生成任务承诺**：Web Dapp 为每张卡的每个任务生成一个预映像 `n`，把对应的哈希写进合约（`batchDepositWithTasks`）。预映像由你掌控——可托管在 Hongbao Web，也可导出到你自己的后台

> 任务卡总额 = 基础份额 + Σ 任务份额。任务槽创建后不可变；续充只进基础份额。机制详见 [dev/contract.md § 任务卡](../dev/contract.md)。
...
链上交易通过 Hongbao 官方 Relayer（默认）或自托管 Relayer 提交。

**任务卡的领取分两段**：用户先扫码、按键领基础份额（这一步同时把收款地址锁定在卡上）；之后每完成一个任务，凭任务凭证 `n` 解锁对应份额，资金自动流向已锁定的地址。任务完成判定由你掌控（Hongbao 托管 or 自建后台）。
